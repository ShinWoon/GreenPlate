...
class StampAdapter (val context : Context, private var items : List<Boolean>) : RecyclerView.Adapter<StampAdapter.StampViewHolder>(){
    ...
    inner class StampViewHolder(private val binding: ItemStampRecyclerViewBinding) : RecyclerView.ViewHolder(binding.root) {
        fun bindInfo(stamp : Boolean) {
            if(stamp) {
                // true 상태 -> stamp 찍어 주기
                binding.stampCircleView.background = ContextCompat.getDrawable(context, R.drawable.stamp_circle)
                binding.stampLogoView.background = ContextCompat.getDrawable(context, R.drawable.logo_color)
            } else {
                // false -> stamp 안 찍힌 상태
                binding.stampCircleView.background = ContextCompat.getDrawable(context, R.drawable.no_stamp_circle)
                binding.stampLogoView.background = ContextCompat.getDrawable(context, R.drawable.logo_gray)
            }
        }
    }
    ...
    override fun onBindViewHolder(holder: StampAdapter.StampViewHolder, position: Int) {
