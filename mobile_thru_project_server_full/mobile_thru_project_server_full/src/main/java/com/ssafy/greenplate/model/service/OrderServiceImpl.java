...
public class OrderServiceImpl implements OrderService {
    ...
    public void makeOrder(Order order) {
        ...
        // 스템프 정보 저장
        // Stamp stamp =
        // Stamp.builder().userId(order.getUserId()).quantity(quantitySum).orderId(order.getId()).build();
        Stamp stamp = new Stamp(order.getUserId(), order.getId(), quantitySum);
        sDao.insert(stamp);

        // 사용자 정보 업데이트
        // User user =
        // User.builder().id(order.getUserId()).stamps(stamp.getQuantity()).build();
        User user = new User();
	...
	@Override
	public int updateCompletedYes(int orderId) {
		return oDao.updateCompletedYes(orderId);
	}
